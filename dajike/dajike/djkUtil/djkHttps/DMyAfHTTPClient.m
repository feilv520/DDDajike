...
static NSString * const AFPortStr = @"/djks/call";
...
@implementation DMyAfHTTPClient
{
...
+ (instancetype)sharedClient {
    static DMyAfHTTPClient *_sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedClient = [[DMyAfHTTPClient alloc] initWithBaseURL:[NSURL URLWithString:DMYAFHTTP_BASEURL]];
...
- (void)uploadImageRequestWithUrl:(NSString *)method andImagePaths:(NSDictionary *)paths andDic:(NSDictionary *)params success:success failure:failure
{
    ...
    NSLog(@"url = %@",[NSString stringWithFormat:@"%@%@",DMYAFHTTP_BASEURL,AFPortStr]);
    
    _request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",DMYAFHTTP_BASEURL,AFPortStr]]];
    ...
    __weak DMyAfHTTPClient *weakSelf0 = self;
    ...
    __weak DMyAfHTTPClient *weakSelf = self;
...
{
    NSString *urlStr = [NSString stringWithFormat:@"%@%@%@",DMYAFHTTP_BASEURL,AFPortStr,oldUrlStr];
