...
- (void)addData:(void (^)(BOOL finish))success{
    if (self.collectType == GOODS) {
        NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:get_userId,@"userId",[NSString stringWithFormat:@"%ld",_goodPage],@"page",@"10",@"pageSize", nil];
        ...
        NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:get_userId,@"userId",[NSString stringWithFormat:@"%ld",_shopPage],@"page",@"10",@"pageSize",[FileOperation getLatitude],@"latitude",[FileOperation getLongitude],@"longitude",nil];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_rightBut.selected) {
        ...
        if (self.collectType == GOODS) {
            ...
            DCollectGoodModel *goodsListModel = [goodDataArray objectAtIndex:indexPath.row];
