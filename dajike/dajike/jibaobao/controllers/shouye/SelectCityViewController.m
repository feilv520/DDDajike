...
@interface SelectCityViewController ()<BATableViewDelegate,UISearchBarDelegate>
{
...
//    self.mainTabview.allowsSelection=YES;
//    self.mainTabview.sectionIndexMinimumDisplayRowCount = 8;
//    self.mainTabview.allowsMultipleSelection = YES;
//    //[myTableView setScrollEnabled:NO];
//    self.mainTabview.showsHorizontalScrollIndicator = YES;
//    self.mainTabview.showsVerticalScrollIndicator = YES;
//    //[XtomFunction addbordertoView:myTableView radius:8.0f width:0.0f color:BB_White_Color];
//    //设置索引列文本的颜色
//    self.mainTabview.sectionIndexColor = Color_gray4;
//    self.mainTabview.sectionHeaderHeight = 40;
//    self.mainTabview.sectionFooterHeight = 50;
//    self.mainTabview.sectionIndexBackgroundColor=Color_gray1;
...
{
    _searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(15, 5, WIDTH_CONTROLLER_DEFAULT-20-10, 30)];
//    searchBar.backgroundColor = Color_White;
    _searchBar.barTintColor = Color_White;
    _searchBar.layer.borderColor = [Color_gray2 CGColor];
    _searchBar.layer.borderWidth = 1.0;
    _searchBar.delegate = self;
    [self.view addSubview:_searchBar];
    
//    CGRect frame = self.mainTabview.frame;
//    frame.origin.y = self.mainTabview.frame.origin.y+40;
//    frame.size.height = self.mainTabview.frame.size.height-40;
//    [self.mainTabview setFrame:frame];
    self.contactTableView = [[BATableView alloc] initWithFrame:CGRectMake(0, 40, WIDTH_CONTROLLER_DEFAULT, HEIGHT_CONTROLLER_DEFAULT-40-64)];
...
- (void)addData
{
    ...
    [suoyinCityList insertObject:@"最近" atIndex:1];
    [suoyinCityList insertObject:@"热门" atIndex:2];
//    [suoyinCityList insertObject:@"全部" atIndex:3];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //从plist文件中获取所有城市列表
        NSArray *citysArr = [FileOperation getAllPlaces];
        //添加完首字母的城市列表
            ...
            
        {
            ...
            for (CityModel *model0 in resultCityArr) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //获取热门城市
        [[MyAfHTTPClient sharedClient] postPathWithMethod:@"GuangGaoHtmls.show" parameters:@{@"code":@"jbb_index_chooseCity"} ifAddActivityIndicator:NO success:^(AFHTTPRequestOperation *operation, PostData *responseObject) {
...
- (NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if ( section == 0||section == 1||section == 2) {
        ...
        if (dataArray.count > 0) {
            return ((NSArray *)[dataArray objectAtIndex:section-3]).count;
        }
        return 0;
    }
    return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((searchCitys !=nil)&&(searchCitys.count > 0)) {
        ...
        UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, 29, WIDTH_CONTROLLER_DEFAULT, 1)];
        line.backgroundColor = Color_gray2;
        [cell3 addSubview:line];
        cell3.textLabel.text = [[searchCitys objectAtIndex:indexPath.row] objectForKey:@"regionName"];
        return cell3;
    }
    if (section == 0) {
        UITableViewCell *cell0 = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cell0"];
        cell0.backgroundColor = Color_tableV;
        cell0.contentView.backgroundColor = [UIColor clearColor];

        
        cityLabel = [[UILabel alloc]creatLabelWithFrame:CGRectMake(15, 0, WIDTH_CONTROLLER_DEFAULT-50, 30) AndFont:13 AndBackgroundColor:Color_White AndText:[NSString stringWithFormat:@" 定位城市：%@",[FileOperation getDingweiCityName]] AndTextAlignment:NSTextAlignmentLeft AndTextColor:Color_Black andCornerRadius:2.0];
        cityLabel.layer.borderColor = [Color_gray2 CGColor];
    ...
    }else if(section == 1)
    {
        ...
        cell1.backgroundColor = Color_tableV;
        
        NSArray *recentlyCitys = [FileOperation getRecentlyCitys];
        for (int i = 0; i < recentlyCitys.count-1; i++) {
            UIButton *btn = [[UIButton alloc] createButtonWithFrame:CGRectMake(15+(i%3)*70, 0, 70.5, 30) andBackImageName:nil andTarget:self andAction:@selector(cell1ButtonClip:) andTitle:[[recentlyCitys objectAtIndex:i+1]objectForKey:@"regionName"] andTag:i+100];
            btn.backgroundColor = Color_White;
            btn.layer.borderColor = [Color_gray2 CGColor];
    ...
    else if(section == 2)
    {
        ...
        cell2.backgroundColor = Color_tableV;
        
        if (hostCity.count > 0) {
            for (int i = 0; i < hostCity.count; i++) {
                UIButton *btn = [[UIButton alloc] createButtonWithFrame:CGRectMake(15+(i%4)*69, (i/4)*30, 69.5, 30.5) andBackImageName:nil andTarget:self andAction:@selector(cell2ButtonClip:) andTitle:[[hostCity objectAtIndex:i]objectForKey:@"regionName"] andTag:i+100];
                btn.backgroundColor = Color_White;
                btn.layer.borderColor = [Color_gray2 CGColor];
                btn.layer.borderWidth = 0.5;
                [btn setTitleColor:Color_Black forState:UIControlStateNormal];
       ...
       static NSString *identifer = @"cell3";
        ...
        UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, 29, WIDTH_CONTROLLER_DEFAULT, 0.5)];
        line.backgroundColor = Color_gray2;
        [cell3 addSubview:line];
        if (dataArray.count > 0) {
            cell3.textLabel.text = ((CityModel *)[[dataArray objectAtIndex:section-3] objectAtIndex:indexPath.row]).regionName;
        }
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section >= 3) {
        //写进第一项中去
        [FileOperation writeCurrentCityWithCityName:((CityModel *)[[dataArray objectAtIndex:section-3] objectAtIndex:row]).regionName andCityId:[NSString stringWithFormat:@"%@",((CityModel *)[[dataArray objectAtIndex:section-3] objectAtIndex:row]).regionId]];
        if ([FileOperation getNextPlacesByCurrentCityId:[NSString stringWithFormat:@"%@",((CityModel *)[[dataArray objectAtIndex:section-3] objectAtIndex:row]).regionId]].count > 0) {
            [FileOperation setPlistObject:[FileOperation getNextPlacesByCurrentCityId:[NSString stringWithFormat:@"%@",((CityModel *)[[dataArray objectAtIndex:section-3] objectAtIndex:row]).regionId]] forKey:kCurrentCityArr ofFilePath:[FileOperation creatPlistIfNotExist:SET_PLIST]];
        }
...
- (void) cell1ButtonClip:(id) sender
{
    ...
    //获取最近访问城市的信息
    NSArray *cityArrs = [FileOperation getRecentlyCitys];
...
- (void) cell2ButtonClip:(id) sender
{
    ...
    //写进第一项中去
    [FileOperation writeCurrentCityWithCityName:[[hostCity objectAtIndex:index]objectForKey:@"regionName"] andCityId:[NSString stringWithFormat:@"%@",[[hostCity objectAtIndex:index]objectForKey:@"regionId"]]];
    if ([FileOperation getNextPlacesByCurrentCityId:[NSString stringWithFormat:@"%@",[[hostCity objectAtIndex:index]objectForKey:@"regionId"]]].count > 0) {
        [FileOperation setPlistObject:[FileOperation getNextPlacesByCurrentCityId:[NSString stringWithFormat:@"%@",[[hostCity objectAtIndex:index]objectForKey:@"regionId"]]] forKey:kCurrentCityArr ofFilePath:[FileOperation creatPlistIfNotExist:SET_PLIST]];
    }
    //返回
    [self.navigationController popViewControllerAnimated:YES];
