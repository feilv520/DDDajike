...
const CGFloat kFilterViewHeight = 54.0;
@interface PYTabbar ()
{
        ...
        andFrame:(CGRect)frame containerView:(UIView *)contrainerView
{
    ...
    if (self) {
        ...
        _topBackgroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _backgroundView.frame.size.width, 2)];
        _topBackgroundView.backgroundColor = [UIColor whiteColor];
        //        NSLog(@"注意注意%f",_selectedBackgroundView.frame.size.height-5);
        [self.backgroundView addSubview:self.topBackgroundView];
        CGFloat x = 0;
        CGFloat buttonWidth = self.frame.size.width/(strings.count-1);
        NSInteger tag = 0;
        _selectedBackgroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, buttonWidth, frame.size.height)];
        _selectedBackgroundImageView = [[UIImageView alloc]initWithFrame:_selectedBackgroundView.frame];
        [self.selectedBackgroundView addSubview:self.selectedBackgroundImageView];
        _selectedTopBackgroundView = [[UIView alloc]initWithFrame:CGRectMake(15, 0, _selectedBackgroundView.frame.size.width-30, 2)];
//        NSLog(@"注意注意%f",_selectedBackgroundView.frame.size.height-5);
        [self.selectedBackgroundView addSubview:self.selectedTopBackgroundView];
        [self addSubview:self.selectedBackgroundView];
        
        for (int i = 0;i < strings.count-1;i++) {
            NSString *string = [NSString stringWithFormat:@"%@",[strings objectAtIndex:i]];
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            [button setTag:tag];
            [button setFrame:CGRectMake(x, 0, buttonWidth, self.frame.size.height)];
            [button setTitle:string forState:UIControlStateNormal];
//            //设置tabbar边框阴影
//            button.layer.shadowColor = [UIColor blackColor].CGColor;
//            button.layer.shadowOffset = CGSizeMake(5.0, 5.0);
//            button.layer.shadowOpacity = YES;
            [button addTarget:self action:@selector(onButton:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:button];
             x += buttonWidth;
            tag  += 1;
        }
        NSString *string = [NSString stringWithFormat:@"%@",[strings objectAtIndex:strings.count-1]];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        [button setTag:tag];
        [button setFrame:CGRectMake(x, 0, 0.1, self.frame.size.height)];
        [button setTitle:string forState:UIControlStateNormal];
        //            //设置tabbar边框阴影
//        button.layer.shadowColor = [UIColor blackColor].CGColor;
//        button.layer.shadowOffset = CGSizeMake(5.0, 5.0);
...
- (void)applyDefaultStyle
{
    ...
    [self setSelectedItemBackgroundColor:[UIColor whiteColor]];
    [self setSelectedItemTopBackgroundColor:[UIColor colorWithRed:203/255.0 green:20/255.0 blue:72/255.0 alpha:1]];
...
- (void)onButton:(id)sender
{
    ...
    [button setTitleColor:[UIColor colorWithRed:203/255.0 green:20/255.0 blue:72/255.0 alpha:1] forState:UIControlStateNormal];
    CGFloat animationSpeed;
