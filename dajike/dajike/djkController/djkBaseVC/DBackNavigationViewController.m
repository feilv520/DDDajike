//
//  DBackNavigationViewController.m
//  dajike
//
...
@interface DBackNavigationViewController ()<UITableViewDelegate>
@property (nonatomic, readonly) DBaseNavView *m_viewNaviBar;

@end

@implementation DBackNavigationViewController
...
- (void) viewWillDisappear:(BOOL)animated
{
    ...
    //底部tabBar显现
    SWBTabBarController *jTabBarVC = [SWBTabBarController sharedManager];
    [jTabBarVC.myView setHidden:NO];
...
{
    self.dMainTableView = [[UITableView alloc]initWithFrame:DRect(0, 44, DWIDTH_CONTROLLER_DEFAULT, DHEIGHT_CONTROLLER_DEFAULT) style:UITableViewStyleGrouped];
...
- (void) changeNavTitleColor:(UIColor *)color
{
    if (_viewNaviBar)
    {
        [_viewNaviBar setNavTitleColor:color];
    }else{APP_ASSERT_STOP}
}
...
- (void)navigationCanDragBack:(BOOL)bCanDragBack
{
    if (self.navigationController)
    {
        [((DBackNavigationViewController *)(self.navigationController)) navigationCanDragBack:bCanDragBack];
    }else{}
}
