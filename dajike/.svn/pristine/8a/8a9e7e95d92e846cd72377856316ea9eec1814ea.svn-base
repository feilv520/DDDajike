...
+ (FMDatabase *)getDatabase
{
    ...
    [db executeUpdate:@"CREATE TABLE IF NOT EXISTS AddGouWuChe (status integer, goodsId text, goodsPrice integer, number integer, goodsDefaultImage text, goodsName text, spec1 text, spec2 text, specId integer, specName1 text, specName2 text, storeId integer, storeLogo text, storeName text, stock integer, lastTime text)"];
    return db;
}
//添加到本地购物车
+ (void)addIntoGouWuChe:(DGoodsDetailModel *)model
{
    ...
    //如果数据库已有该商品，合并更新数量
    NSString *sqlUpdate = [NSString stringWithFormat:@"UPDATE AddGouWuChe SET number = number+%d,status='0' WHERE specId ='%d' AND storeId ='%d'",[model.number intValue],[model.specId intValue],[model.storeId intValue]];
    //如果数据库没有该商品，执行插入
    NSString *sqlInsert = [NSString stringWithFormat:@"INSERT INTO AddGouWuChe (status, goodsId, goodsPrice, number, goodsDefaultImage, goodsName, spec1, spec2, specId, specName1, specName2, storeId, storeLogo, storeName, stock, lastTime) VALUES ('%d','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@')",0,model.goodsId,model.goodsPrice,model.number,model.goodsDefaultImage,model.goodsName,model.spec1,model.spec2,model.specId,model.specName1,model.specName2,model.storeId,model.storeLogo,model.storeName,model.stock,time_stamp];
...
+ (NSMutableDictionary *)traverseGouWuChe
{
    ...
    for (int i=0; i<storeIdKeyArr.count; i++) {
        ...
        while ([s next]) {
            ...
            [goodsDic setObject:[NSString stringWithFormat:@"%d",[s intForColumn:@"number"]] forKey:@"number"];
            [goodsDic setObject:[NSString stringWithFormat:@"%d",[s intForColumn:@"stock"]] forKey:@"stock"];
...
+ (int)getGouWuCheNum
{
    FMDatabase *db = [self getDatabase];
    NSMutableArray *storeIdKeyArr = [[NSMutableArray alloc]init];
    FMResultSet *s = [db executeQuery:@"SELECT storeId FROM AddGouWuChe"];
    while ([s next]) {
        [storeIdKeyArr addObject:[NSString stringWithFormat:@"%d",[s intForColumn:@"storeId"]]];
    }
    NSLog(@"%@",storeIdKeyArr);
    [db close];
    return (int)storeIdKeyArr.count;
}
