...
- (void)setGoodsDetailModel:(DGoodsDetailModel *)goodsDetailModel
{
    ...
    if (self.yunfei && ![self.yunfei isEqualToString:@"0.00"]) {
    ...
    CGRect rect2 = [self contentAdaptionLabel:self.storeLb.text withSize:CGSizeMake(DWIDTH_CONTROLLER_DEFAULT-95-self.yunfeiLbCon.constant, 20) withTextFont:13.0f];
    ...
    if ([self.flag isEqualToString:@"jiliduihuan"]) {
        self.shifukuanLb.text = [NSString stringWithFormat:@"小计：%d积分",[goodsDetailModel.ifJifen intValue]*[self.buyNum intValue]];
    }else
...
{
    self.storeLb.text = self.storeName;
    if (self.yunfei && ![self.yunfei isEqualToString:@"0.00"]) {
        self.yunfeiLb.text = [NSString stringWithFormat:@"运费：%.2f",[self.yunfei floatValue]];
    }else
        self.yunfeiLb.text = @"免运费";
    ...
    self.storeLbCon.constant = rect2.size.width+5;
    [self.storeLogoImg setImageWithURL:[commonTools getImgURL:self.storeLogo] placeholderImage:DPlaceholderImage];
    self.shifukuanLb.text = [NSString stringWithFormat:@"小计：%.2f",[self.xiaoji floatValue]];
    CGRect rect3 = [self contentAdaptionLabel:self.shifukuanLb.text withSize:CGSizeMake(500, 15) withTextFont:12.0f];
    self.shifukuanCon.constant = rect3.size.width+5;
    self.productNumLb.text = [NSString stringWithFormat:@"共%@件",self.buyNum];
}
