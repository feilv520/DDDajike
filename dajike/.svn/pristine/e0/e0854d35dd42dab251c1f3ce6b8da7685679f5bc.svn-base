...
//        webViewiew.delegate = self;
    ...
    if (self.type == IS_NEED_ADD) {
        if (![commonTools isNull:self.goodId]) {
    ...
    if ([FileOperation isLogin]) {
        ...
        [[DMyAfHTTPClient sharedClient]postPathWithMethod:@"Carts.get" parameters:parameter ifAddActivityIndicator:NO success:^(AFHTTPRequestOperation *operation, PostData *responseObject) {
            ...
            if (responseObject.succeed) {
                ...
                [webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"getCartCount(%@)",[responseObject.result objectForKey:@"specCount"]]];
            }else
        ...
        if ([FileOperation isLogin]) {
            ...
            [[DMyAfHTTPClient sharedClient]postPathWithMethod:@"Carts.saveOrGet" parameters:parameter ifAddActivityIndicator:NO success:^(AFHTTPRequestOperation *operation, PostData *responseObject) {
                ...
                if (responseObject.succeed) {
                    ...
                    [FileOperation setPlistObject:[NSString stringWithFormat:@"%d",goodNum] forKey:@"cartCount" ofFilePath:[FileOperation creatPlistIfNotExist:jibaobaoUser]];
...
- (void) navRightButtonTapped:(id) sender
{
    ...
    NSString *userIdStr = [self.urlStr substringFromIndex:8];
//    if ([FileOperation isLogin]) {
