...
@interface DHongBaoShareViewController () <UIAlertViewDelegate>

@end

@implementation DHongBaoShareViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    
    [self setNaviBarTitle:@"给小伙伴发红包"];
...
#warning 需要修改此处的 userId [FileOperation getUserId]
    NSDictionary *parameter = @{@"userId":[FileOperation getUserId],@"hongbaoId":[NSString stringWithFormat:@"%@", self.hongBaoId]};
    [[DMyAfHTTPClient sharedClient]postPathWithMethod:@"MyHongBaos.cancel" parameters:parameter ifAddActivityIndicator:NO success:^(AFHTTPRequestOperation *operation, PostData *responseObject) {
