...
{
//    NSString *filename=[self creatPlistIfNotExist:filePath];
    NSArray *paths=NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES);
    NSString *path=[paths    objectAtIndex:0];
...
#pragma mark-------------------
#pragma mark-----------关于最近浏览商品（猜你喜欢）--------------------------
+ (NSMutableArray *)getCurrentGoodsIdsArr
{
    NSMutableArray *tmpArr = [[FileOperation getobjectForKey:kRecentlyGoodsIdArr ofFilePath:[FileOperation creatPlistIfNotExist:SET_PLIST]]mutableCopy];
    if (tmpArr == nil) {
        tmpArr = [[NSMutableArray alloc]initWithCapacity:10];
    }
    return tmpArr;
}
//获得浏览过的商品Id 长字符串
...
{
    NSMutableArray *tmpArr = [FileOperation getCurrentGoodsIdsArr];
    NSString *str = [tmpArr componentsJoinedByString:@","];
    return [commonTools isNull:str]?@"":str;
}
//
+ (void)addGoodIdToCurrentGoodsIdsArr:(NSString *)goodId
{
    ...
    [tmpArr enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isEqualToString:goodId]) {
            [tmpArr removeObjectAtIndex:idx];//如有重复，移除旧的，添加新的
            *stop = YES;
        }
    }];
    [tmpArr insertObject:goodId atIndex:0];
    NSArray *resultArr = [[NSArray alloc]init];
    if (tmpArr.count>10) {
        NSRange range = NSMakeRange(0, 10);
        resultArr = [tmpArr subarrayWithRange:range];
    }else
    ...
    [mdic setObject:resultArr forKey:kRecentlyGoodsIdArr];
