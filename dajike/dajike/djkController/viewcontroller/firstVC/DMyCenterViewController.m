...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        ...
        if ([FileOperation isLogin]) {
            ...
            self.nunLab_1.text = @"0";
            self.numLab_2.text = @"0";
        }
...
- (void)myAccontButAction:(UIButton *)but{
    ...
    if (but.tag == 1) {
        ...
        vc.totalAccount = [commonTools moneyTolayout:_accountModel.chongzhi_jine_index];
    }else if (but.tag == 2){
        vc.AccountDetailType = ZHANGHU;
        vc.totalAccount = [commonTools moneyTolayout:_accountModel.shouyi_yue];
    }else if (but.tag == 3){
