...
@property (nonatomic, strong) LocationErrorStringBlock errorStringBlock;
...
//获取经纬度
- (void) getLocationCoordinate:(LocationBlock) locaiontBlock andLocationErrer:(LocationErrorStringBlock)locationErrorStringBlock
{
    self.locationBlock = [locaiontBlock copy];
    self.errorStringBlock = [locationErrorStringBlock copy];
    [self startLocation];
}
//获取经纬度
...
-(void)startLocation
{
    ...
    else
    {
        ...
        [alvertView show];
        if (_errorStringBlock) {
            _errorStringBlock(@"需要开启定位服务");
            _errorStringBlock = nil;
        }
